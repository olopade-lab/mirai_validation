...
def auc_calculator(df, bootstraps):
    ...
    def split_set(df, case_col):
        ''' 
        splits set to preserve case/control proportion 
        '''
        cases = df[df[case_col] == True] #for each year AUC, you want representation from the appropriate case column
        controls = df[df[case_col] == False]
        sample_cases= cases.sample(frac=0.15, replace=False)
        sample_controls = controls.sample(frac= 0.15, replace=False)
        df_sample= pd.concat([sample_cases, sample_controls])
        return df_sample
    ...
    def get_aucs3yr(df):
        ...
        fpr, tpr, thresholds = metrics.roc_curve(y, pred, pos_label=True)
        auc3yr = metrics.auc(fpr,tpr)
        return auc3yr

    def get_aucs4yr(df):
        y = get_column(df, 4, 'case_4yr')
        pred = get_column(df, 4, '4_year_risk')
        fpr, tpr, thresholds = metrics.roc_curve(y, pred, pos_label=True)
        auc4yr = metrics.auc(fpr,tpr)
        return auc4yr
    ...
    for _ in range(bootstraps):
        df_sample = split_set(df, case_col='case_1yr')
        auc1yr = get_aucs1yr(df=df_sample)
        aucs1yr.append(auc1yr)
    one = mean_confidence_interval(aucs1yr)

    aucs2yr = []
    for _ in range(bootstraps):
        df_sample = split_set(df, case_col='case_2yr')
        auc2yr = get_aucs2yr(df=df_sample)
        aucs2yr.append(auc2yr)
    two = mean_confidence_interval(aucs2yr)

    aucs3yr = []
    for _ in range(bootstraps):
        df_sample = split_set(df, case_col='case_3yr')
        auc3yr = get_aucs3yr(df=df_sample)
        aucs3yr.append(auc3yr)
    three = mean_confidence_interval(aucs3yr)

    aucs4yr = []
    for _ in range(bootstraps):
        df_sample = split_set(df, case_col='case_4yr')
        auc4yr = get_aucs4yr(df=df_sample)
        aucs4yr.append(auc4yr)
    four = mean_confidence_interval(aucs4yr)

    aucs5yr = []
    for _ in range(bootstraps):
        df_sample = split_set(df, case_col='case_5yr')
        auc5yr = get_aucs5yr(df=df_sample)
        aucs5yr.append(auc5yr)
    five = mean_confidence_interval(aucs5yr)
    print('1-year: {} \n2-year: {}\n3-year: {}\n4-year: {}\n5-year: {}'.format(
        one, two, three, four, five)
    )
    
    print('=' * 60)
    print('{} case-exams at 1 year of {}'.format(case_exams_at_1yr, len(set(get_column(df,1,'exam_id')))))
    print('{} case-exams at 2 years of {}'.format(case_exams_at_2yr, len(set(get_column(df,2,'exam_id')))))
    print('{} case-exams at 3 years of {}'.format(case_exams_at_3yr, len(set(get_column(df,3,'exam_id')))))
    print('{} case-exams at 4 years of {}'.format(case_exams_at_4yr, len(set(get_column(df,4,'exam_id')))))
    print('{} case-exams at 5 years of {}'.format(case_exams_at_5yr, len(set(get_column(df,5,'exam_id')))))
    return print('DONE')
...
def compute_concordance_index(df,bootstraps):
    ...
    def split_set(df):
        ...
        splits set to preserve case/control proportion 
        '''
        cases = df[df['event'] == True]
        controls = df[df['event'] == False]
        sample_cases= cases.sample(frac=0.15, replace=False)
        sample_controls = controls.sample(frac= 0.15, replace=False)
        df_sample= pd.concat([sample_cases, sample_controls])
        return df_sample
    ...
    for _ in range(bootstraps):
        ...
        risk = np.array(sample['risk']) 
        c_index = cic(event_indicator, event_time, estimate=risk)[0]
        c_index_list.append(c_index)
    # print(c_index_list)
    c_index = mean_confidence_interval(data=c_index_list, confidence=0.95)
    return print(f'C-index & Confidence intervals: {c_index}')
